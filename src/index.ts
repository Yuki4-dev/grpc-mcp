    ...
    config: z
      .object({
        deadLine: z
          .number()
          .optional()
          .describe("Deadline for the request in milliseconds"),
        SSL: z
          .boolean()
          .optional()
          .describe("Whether to use SSL for the request"),
      })
      .optional()
...
server.addTool({
  ...
  parameters: z.object({
    dir: z
      .string()
      .describe("Directory containing the proto file (e.g., /path/to/proto)"),
  }),
  execute: async (args) => {
    try {
...
server.addTool({
  ...
  execute: async (args) => {
    try {
      const res = await loader.getMethodAsync(
        args.path,
        args.service,
        args.method,
      );
