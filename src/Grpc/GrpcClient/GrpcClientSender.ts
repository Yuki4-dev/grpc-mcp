...
import {
  isErrorStatusObject,
  ProtocolBufferErrorCode,
  toGrpcStatusCode,
} from "../ProtocolBuffer/ProtocolBuffer.js";
import { IProtoLoader, protoLoader } from "../ProtocolBuffer/ProtoLoader.js";
import {
  ClientCallOptions,
  ClientOptions,
  SendGrpcClientRequest,
  SendGrpcClientResponse,
} from "./GrpcClient.js";
import { grpcClientFactory, IGrpcClientFactory } from "./GrpcClientFactory.js";
...
class GrpcClientSender implements IGrpcClientSender {
  ...
  constructor(
    private readonly protoLoader: IProtoLoader,
    private readonly grpcClientFactory: IGrpcClientFactory,
  ) {}

  async sendAsync(
    request: SendGrpcClientRequest,
  ): Promise<SendGrpcClientResponse> {
    ...
    if (loadProto.length !== 1) {
      Logger.warn(
        "PostGrpcRequest use file path. Not directory path : " + reqProtoPath,
      );
    }

    let option: ClientOptions | undefined;
    ...
    const client = this.grpcClientFactory.create(
      request.address.host +
        (request.address.port ? ":" + request.address.port : ""),
    ...
    let callOption: ClientCallOptions | undefined;
    ...
    let callResult;
    try {
      callResult = await client.callAsync(
        request.request.method,
        string2Obj(request.request.body),
      ...
      return this.toErrorResponse(
        ...
        "UNKNOWN",
        `GrpcClient call failed: ${ex.message}`,
      );
    } finally {
    ...
    return {
      analyze,
      response: { ok: true, body: obj2String(callResult, true) },
    };
  }
  ...
  private validateRequest(request: SendGrpcClientRequest): string[] {
    ...
    if (
      request.address.port !== undefined &&
      (request.address.port < 1 || Number.isNaN(request.address.port))
    ) {
  ...
  private cleanCallResult(result: any): any | undefined {
  ...
  private createCancellationTokenSource(): CancellationTokenSource {
    const tokenSource = new CancellationTokenSource();
    this.runTokenSourceList = this.runTokenSourceList.filter(
      (ts) => !ts.token.isCancellationRequested,
    );
...
export const grpcClientSender: IGrpcClientSender = new GrpcClientSender(
  protoLoader,
  grpcClientFactory,
);
