...
type ClientCallCallback = (
  error: grpc.StatusObject | undefined,
...
class GrpcClient implements IGrpcClient {
  ...
  constructor(
    ...
    if (!this.checkProto(serviceName, proto)) {
      throw new Error(
        `Not contains service: ${serviceName} ${this.getProtoInformation(proto)}`,
      );
    }
  ...
  callAsync(
    ...
    if (!this.checkClient(method)) {
      throw new Error(
        `Not contains method: ${method} ${this.getProtoInformation()}`,
      );
    }

    const callInfo =
      this.proto.protocolBuffer.packageName +
      "." +
      this.serviceName +
      "/" +
      method;
  ...
  private checkProto(
    ...
    if (
      !Object.keys(proto.packageDefinition).includes(
  ...
  private createMetaData(
    ...
    if (options?.deadlineMsec) {
      metadata.set(
        "deadline",
        new Date()
          .setMilliseconds(new Date().getMilliseconds() + options?.deadlineMsec)
          .toString(),
      );
    }
    return metadata;
