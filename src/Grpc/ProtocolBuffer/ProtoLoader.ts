...
type ProtocolBufferServiceWithPackage = ProtocolBufferService & {
  packageName: string;
};
...
class ProtoLoader implements IProtoLoader {
  async loadAsync(
    fileOrDir: string,
  ): Promise<ProtocolBufferPackageDefinition[]> {
  ...
  async loadProtoAsync(path: string): Promise<ProtocolBufferPackageDefinition> {
    ...
    const protoFileName =
      path.replace(proto_ext, "").split(separator).pop() +
      "." +
      path.split(".").pop();
    ...
    const { packageName, services, messages, enums } =
      this.getProtocolBufferServiceMessageEnum(packageDefinition);
      ...
      Object.keys(proto).forEach((mk) => {
        ...
        methods.push({
          name: method.originalName
            ? method.originalName.charAt(0).toUpperCase() +
              method.originalName.slice(1)
            : "",
          path: method.path,
  ...
  getTypeMessage(
    ...
    const fields = messageType.field.map((field) => {
      return {
        ...
        repeated:
          field.label.indexOf("LABEL_REPEATED") > -1
            ? (true as const)
            : undefined,
      };
    });
